...
using TMPro;    // TextMesh Pro 관련 컴포넌트에 접근하기 위한 선언
...

{
    ...
    void Start() 
    {
        ...
        // SpawnPointGroup 하위에 있는 모든 차일드 오브젝트의 Trasnform 컴포넌트 추출
        ...
        // 스코어 점수 출력
        // totScore = PlayerPrefs.GetInt("TOT_SCORE", 0); 이 코드는 직전의 스코어를 저장해준다.
        DisplayScore(0);
    }
    ...
    void CreateMonsterPool()   
    {
        for(int i = 0; i < maxMonsters; i++)
        {
            ...
            // 몬스터의 이름을 지정
            _monster.name = $"Monster_{i:00}";      // 가독성을 위해 문자열 포맷을 지정했다. $ 기호를 사용해 몬자열 치환과 숫자 포맷을 한번에 지정할 수 있다. 숫자의 포맷은 콜론 뒤에
                                                    // 표기하며, 00의 의미는 표시하는 숫자의 자릿수를 두 자리로 유지하고, 숫자의 자릿수가 부족한 경우 0으로 채운다는 뜻.  
            // 몬스터 비활성화
